...
def online_quality_aware(time):
    ...
    while t <= time:
        ...
        for n in nodes:
            ...
            print('\tnode_id=%d rate=%f budget=%f points_num=%d remain=%d all_user_num=%d' % (
                n.id, rate, n.budget, len(n.points), len(n.m_select), len(n.m_departure)))
            if t == time:
                pass
                # csv.write_csv('result33', [n.id, rate, n.budget, len(n.m_departure), task.budget])
        budget_update()
        t += 1
...
def run_task(m_selected, t, node):
    ...
    for m in m_selected:
        ...
        if m.trip_second >= (t - m.arrive_time) * slot:
            count = random.randint(1, int(len(m.interest_points) / T + 0.5))
...
if __name__ == '__main__':
    # init()
    print(task.budget, task.save_budget)
